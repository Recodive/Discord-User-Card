...
export function imageToUrl({
	...
	switch (scope) {
		...
		default: {
			return relatedId ?
				`https://cdn.discordapp.com/${scope}/${relatedId}/${animatedPrefix}${image.id}.${animated ? "gif" : "webp"}` :
				`https://cdn.discordapp.com/${scope}/${animatedPrefix}${image.id}.${animated ? "gif" : "webp"}`;
		}
	}
}
